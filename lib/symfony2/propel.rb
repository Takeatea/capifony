  ...
  namespace :propel do
    namespace :database do
      [:create, :drop].each do |action|
        ...
        task action, :roles => :app, :except => { :no_release => true } do
          ...
          run "#{try_sudo} sh -c 'cd #{latest_release_symfony_path} && #{php_bin} #{symfony_console} propel:database:#{action.to_s} #{console_options}'", :once => true
          capifony_puts_ok
        end
      end
    end
    ...
    namespace :build do
      ...
      task :model, :roles => :app, :except => { :no_release => true } do
        ...
        run "#{try_sudo} sh -c 'cd #{latest_release_symfony_path} && #{php_bin} #{symfony_console} #{command} #{console_options}'"
        capifony_puts_ok
      end
      ...
      task :sql_load, :roles => :app, :except => { :no_release => true } do
        ...
        run "#{try_sudo} sh -c 'cd #{latest_release_symfony_path} && #{php_bin} #{symfony_console} #{command} --force #{console_options}'", :once => true
        capifony_puts_ok
      end
      ...
      task :all_and_load, :roles => :app, :except => { :no_release => true } do
        ...
        run "#{try_sudo} sh -c 'cd #{latest_release_symfony_path} && #{php_bin} #{symfony_console} propel:build --insert-sql #{console_options}'"
        capifony_puts_ok
      end

      desc "Generates ACLs models"
      task :acl, :roles => :app, :except => { :no_release => true } do
        run "#{try_sudo} sh -c 'cd #{latest_release_symfony_path} && #{php_bin} #{symfony_console} propel:acl:init #{console_options}'"
      end

      desc "Inserts propel ACL tables"
      task :acl_load, :roles => :app, :except => { :no_release => true } do
        run "#{try_sudo} sh -c 'cd #{latest_release_symfony_path} && #{php_bin} #{symfony_console} propel:acl:init #{console_options} --force'", :once => true
      end
    end
  end
end
